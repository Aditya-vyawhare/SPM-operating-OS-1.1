...
## Part 4b: Self-Updating Memory

PM OS can automatically learn from your conversations and keep its knowledge base current without manual work. This section sets up the three mechanisms that make memory self-updating.

### Step 1: Install the Continual Learning plugin

The [Continual Learning plugin](https://cursor.com/marketplace/cursor/continual-learning) is an official Cursor marketplace plugin that automatically processes your agent conversations after each session. It extracts durable corrections and workspace facts and writes them into `AGENTS.md`.

**Install:**
- Open Cursor → Settings → Plugins → search "Continual Learning" → Install
- Or type `/add-plugin continual-learning` in any Cursor chat

**What it does:**
- Runs a `stop` hook after every agent conversation
- Mines transcripts for recurring corrections (things you corrected the agent on 2+ times)
- Extracts durable workspace facts (file locations, conventions, data schemas)
- Appends new entries to `AGENTS.md` under Learned User Preferences and Learned Workspace Facts
- Tracks what it has already processed so it never re-reads old transcripts

**No configuration needed.** Once installed, memory accumulates automatically.

### Step 2: Set up the scheduler (optional)

The scheduler runs agent commands on a cron schedule — even when you're not actively chatting. This powers automatic metrics refresh, Slack sync, and daily planning.

1. Run the setup script:
   ```bash
   bash scheduler/setup.sh
   ```
2. Fill in your API keys in `scheduler/.env`
3. The scheduler starts automatically on login (macOS LaunchAgent)

See `config/agent-schedule.md` for the full cadence map.

### Step 3: Configure Cursor IDE cron (alternative to scheduler)

If you prefer Cursor-native scheduling (runs when the IDE is open, no API key needed):

1. Edit `.cursor/agent-schedules.json` — the template has three commands pre-configured
2. Adjust cron times to your timezone and workday
3. Cursor will run the slash commands automatically at the configured times

**Both can coexist.** The headless scheduler handles runs when Cursor is closed; IDE cron handles runs when it's open. Cooldown files (`.cursor/hooks/state/`) prevent duplicate runs.
